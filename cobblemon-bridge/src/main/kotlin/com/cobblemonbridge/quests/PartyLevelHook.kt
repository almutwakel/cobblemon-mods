...
/**
 * Awards `server:reach_party_level_15` the first time the player has any party member at
 * level 15 or above. Fires on two events:
 *   - `LEVEL_UP_EVENT` — a party Pokémon levels up to ≥ 15 from training.
 *   - `POKEMON_CAPTURED` — a Pokémon caught at level ≥ 15 (high-cap zones allow catching above
 *     party max, so a fresh capture can satisfy the quest immediately).
 *
...
object PartyLevelHook {
    ...
    private const val TARGET_LEVEL: Int = 15
    private const val ADVANCEMENT_ID: String = "server:reach_party_level_15"
